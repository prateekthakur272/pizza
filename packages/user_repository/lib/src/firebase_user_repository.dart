...
class FirebaseUserRepository implements BaseUserRepository{
  ...
  Future<MyUser> signUp(MyUser user, String password) async {
    try{
      UserCredential creds = await _firebaseAuth.createUserWithEmailAndPassword(email: user.email, password: password);
  ...
  Stream<MyUser?> get user{
    return _firebaseAuth.authStateChanges().flatMap((user)async*{
      ...
      yield await _userCollection.doc(user!.uid).get().then((data)=> MyUser.fromMap(data.data()!));
    });
  }
